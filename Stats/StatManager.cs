                ...
                using (SqliteDataReader reader = cmd.ExecuteReader())
                {
                    ...
                    if (aggregationLevel == "individual")
                    {
                        while (reader.Read())
                        {
                            ...
                            writer.WriteAttributeString("duration", "" + Math.Ceiling((to - from).TotalSeconds));
                            ...
                            if (!string.IsNullOrEmpty(engagementString))
                            {
                                try
                                {
                                    ...
                                    foreach (Engagement engagement in engagements.Values)
                                    {
                                        ...
                                        writer.WriteAttributeString("duration", "" + Math.Ceiling(engagement.Duration));
                    ...
                    else
                    {
                        ...
                        while (reader.Read())
                        {
                            ...
                            DateTime from = reader.GetDateTime(1);
                            DateTime to = reader.GetDateTime(2);
                            int duration = Convert.ToInt32(Math.Ceiling((to - from).TotalSeconds));
